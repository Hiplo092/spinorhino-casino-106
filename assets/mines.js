...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-ae0e42ec";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-ae0e42ec">'+'<span class="tile-face-ae0e42ec tile-front-ae0e42ec"></span>'+'<span class="tile-face-ae0e42ec tile-back-ae0e42ec"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ae0e42ec")){return;}
var back=tile.querySelector(".tile-back-ae0e42ec");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-ae0e42ec");if(isMine){tile.classList.add("boom-ae0e42ec");}
if(dim){tile.classList.add("dimmed-ae0e42ec");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-ae0e42ec")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-ae0e42ec");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ae0e42ec")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-ae0e42ec");void panelEl.offsetWidth;panelEl.classList.add("jolt-ae0e42ec");}
endRound();}
